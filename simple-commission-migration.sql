-- Simple Commission Tables Migration
-- This script creates the commission tables step by step with error handling
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customers') THEN
        ALTER TABLE commission_earnings ADD CONSTRAINT IF NOT EXISTS fk_commission_earnings_customer_id 
            FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'companies') THEN
        ALTER TABLE commission_earnings ADD CONSTRAINT IF NOT EXISTS fk_commission_earnings_operator_company_id 
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customer_machines') THEN
        ALTER TABLE commission_earnings ADD CONSTRAINT IF NOT EXISTS fk_commission_earnings_customer_machine_id 
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customers') THEN
        ALTER TABLE commission_cashouts ADD CONSTRAINT IF NOT EXISTS fk_commission_cashouts_customer_id 
            FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        ALTER TABLE commission_cashouts ADD CONSTRAINT IF NOT EXISTS fk_commission_cashouts_paid_by 
            FOREIGN KEY (paid_by) REFERENCES users(id);
    END IF;
    
    -- Add foreign keys for commission_payouts
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customers') THEN
        ALTER TABLE commission_payouts ADD CONSTRAINT IF NOT EXISTS fk_commission_payouts_customer_id 
            FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'companies') THEN
        ALTER TABLE commission_payouts ADD CONSTRAINT IF NOT EXISTS fk_commission_payouts_operator_company_id 
            FOREIGN KEY (operator_company_id) REFERENCES companies(id) ON DELETE CASCADE;
    END IF;
...
ALTER TABLE commission_earnings ADD CONSTRAINT IF NOT EXISTS chk_commission_earnings_type 
    CHECK (commission_type IN ('owner', 'referral'));

ALTER TABLE commission_earnings ADD CONSTRAINT IF NOT EXISTS chk_commission_earnings_status 
    CHECK (status IN ('earned', 'pending_cashout', 'paid'));

ALTER TABLE commission_cashouts ADD CONSTRAINT IF NOT EXISTS chk_commission_cashouts_status 
    CHECK (status IN ('pending', 'approved', 'rejected', 'paid'));

ALTER TABLE commission_payouts ADD CONSTRAINT IF NOT EXISTS chk_commission_payouts_status 
