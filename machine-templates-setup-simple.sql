-- Machine Templates Database Setup (Simplified Version)
...
-- Step 2: Create machine_categories table
...
-- Step 3: Create machine_templates table
CREATE TABLE IF NOT EXISTS machine_templates (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    name TEXT NOT NULL,
    category_id UUID REFERENCES machine_categories(id) ON DELETE RESTRICT,
    image_url TEXT,
    dimensions TEXT, -- Format: "LxWxH" in inches
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Step 4: Create machine_template_slots table
...
-- Step 5: Create company_machine_templates table
...
-- Step 6: Create company_machine_template_slots table
...
-- Step 8: Create RLS Policies for machine_categories
...
-- Step 9: Create RLS Policies for machine_templates
...
-- Step 10: Create RLS Policies for machine_template_slots
...
-- Step 11: Create RLS Policies for company_machine_templates
...
-- Step 12: Create RLS Policies for company_machine_template_slots
...
-- Step 15: Database setup complete
-- The machine templates database is now ready for use.
-- Admins can create machine categories and templates through the web interface. 