-- Add Product Categories to VendHub Network (Fixed Version)
-- This script creates product categories and updates the global_products table
-- Compatible with the current schema that uses product_type_id
...
-- Now let's categorize existing products based on their product_type_id
-- We'll join with the product_types table to get the type names
UPDATE global_products 
SET product_category_id = (SELECT id FROM product_categories WHERE name = 'Beverages')
WHERE product_type_id IN (
    SELECT id FROM product_types 
    WHERE name IN ('soda', 'water', 'energy_drink', 'beverage', 'drink')
)
...
WHERE product_type_id IN (
    SELECT id FROM product_types 
    WHERE name IN ('chips', 'snack', 'crackers', 'nuts')
)
AND product_category_id = (SELECT id FROM product_categories WHERE name = 'Other');
...
WHERE product_type_id IN (
    SELECT id FROM product_types 
    WHERE name IN ('candy', 'chocolate', 'sweets', 'confectionery')
)
AND product_category_id = (SELECT id FROM product_categories WHERE name = 'Other');
...
LEFT JOIN global_products gp ON pc.id = gp.product_category_id
